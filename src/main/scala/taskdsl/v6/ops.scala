 ...
 *
 * Contains all syntax from v4 and v5, adding:
 * `hours after` and `is a` syntax for creating short sentences in the builder.
 */
object ops:
    ...
    extension (s: String)
        ...
        def `is a`(task: Task): Task =
            task.copy(id = s)


    /**
     * [[after]] type and value are helpful constructs to make the
     * sentence `10 hours after` work.  The [[after]] value is just an alias for
     * the unit value.
     */
    val after: Unit = ()
    extension (number: Int)
        def hours(after: Unit): TimeModifier =
            TimeModifier.AfterLast(number)
    ...
    private def compileTask(id: String, taskType: TaskType, mods: Chain[TimeModifier | ShippingModifier | RecipientModifier | TaskModifier]): Task =
        mods.foldLeft(Task(id = id, taskType = taskType)):
            ...
            case (task, TimeModifier.AfterLast(hours)) =>
                task.copy(time = TimeAt.HoursAfterLast(hours))
